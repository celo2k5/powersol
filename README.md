...
- The displayed distributable SOL consistently uses the live `state.creatorBalance` from the configured dev wallet. `currentCycle.solUsable` is only a fallback when the backend has not supplied a wallet balance yet.
...
The live ball registry renders every qualifying holder with a one-to-one visual ball ID. The same numbered ball mapping appears in the holder board and winner presentation. During scanning, the registry animates all eligible balls. Once the authoritative transfer arrives, the matching wallet's numbered ball is locked, all other balls dim, and the center-screen winner presentation reveals that ball, wallet, and SOL payout.
